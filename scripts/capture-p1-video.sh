...
FILTER=("$@")
MOTIONS=()
for entry in "${ALL_MOTIONS[@]}"; do
  name="${entry%%:*}"
  if [ ${#FILTER[@]} -eq 0 ]; then
    MOTIONS+=("$entry")
  else
    for f in "${FILTER[@]}"; do [ "$f" = "$name" ] && MOTIONS+=("$entry"); done
  fi
done
...
EOF
  "$FFMPEG" -y -framerate $FPS -i "$dir/%04d.png" -filter_script "$T/filter-$$.txt" -c:v libx264 -crf 20 "$OUT/$name.mp4" > /dev/null 2>&1
