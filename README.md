...
- ✅ **P3 验收满足**：A08 场景（走向椅子坐下全程）+ 触碰矮桌接触交互（**误差实测 max=0.0027H，达标 ≤0.02H**）
- ✅ **P4 开工**：对话 Select 层（规则版离线运行，LLM 版同接口等密钥）、语音适配器（音频时钟+可打断）、说话气泡、**Electron 桌面宿主**（安全基线，`npm run desktop:dev`）
- ✅ 实时性能：完整渲染路径软件渲染下 168 FPS / p95=4.3ms（[perf-baseline](docs/perf-baseline.md)）；30 分钟浸泡零泄漏
- ⏳ 待做：真实 LLM/TTS 接入（等密钥与服务选型，见 `config/README.md`）、透明置顶与多显示器实机验证、A05 拾取/放下完整链（Spec 10.2 允许延后）
